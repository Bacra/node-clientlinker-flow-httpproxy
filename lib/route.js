...
function runAction(linker, action, startTime, body, headers, originalRaw, originalBody)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			if (checkHttpproxyTime(action, startTime, originalBody, options) === false
				|| checkHttpproxyKey(action, headers, originalRaw, originalBody, options) ===  false)
			{
...
function checkHttpproxyTime(action, startTime, originalBody, options)
{
	if (!startTime || !originalBody.time)
	{
		var remain = Math.abs(startTime - originalBody.time);
...
function checkHttpproxyKey(action, headers, originalRaw, originalBody, options)
{
	options || (options = {});
