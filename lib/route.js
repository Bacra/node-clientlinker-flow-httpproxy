...
function httpAction(linker, startTime, req)
{
	...
	return rawBody(req)
		.then(function(buf)
		{
			...
			return runAction(linker, action, body, startTime, originalBody)
				.catch(function(err)
				{
...
function runAction(linker, action, body, startTime, originalBody)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			if (checkHttpproxyKey(action, startTime, originalBody, options) ===  false)
...
function checkHttpproxyKey(action, startTime, originalBody, options)
{
	options || (options = {});
	...
	if (httpproxyKey == originalBody.key)
	{
	...
	try {
		realAction = aes.decipher(originalBody.key, httpproxyKey).split(',');
	}
	catch(err)
	{
		debug('[%s] can not decipher key:%s, err:%o', action, originalBody.key, err);
		return false;
	}

	remain = startTime - realAction.pop();
