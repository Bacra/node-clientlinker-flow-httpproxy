...
function runAction(linker, action, startTime, body, headers, originalRaw, originalBody)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			if (checkHttpproxyKey(action, startTime, headers, originalRaw, originalBody, options) ===  false)
			{
...
function checkHttpproxyKey(action, startTime, headers, originalRaw, originalBody, options)
{
	options || (options = {});
	var httpproxyKey = options.httpproxyKey;
	var httpproxyKeyRemain = options.httpproxyKeyRemain || 2*1000;
	var key = headers['xh-httpproxy-key'];

	debug('[conf] httpproxyKey: %s, remain: %sms', httpproxyKey, httpproxyKeyRemain);
	...
	if (httpproxyKey == key)
	{
		debug('[%s] pass:use originalBody key', action);
		return;
	}

	var targetKey = signature.sha_content(originalRaw, originalBody.time, httpproxyKey);
	if (targetKey != key)
	{
		debug('[%s] not match key:%s, targetKey:%o', action, key, targetKey);
		return false;
	}

	var remain = startTime - originalBody.time;
	if (!startTime || !originalBody.time || remain < 0 || remain > httpproxyKeyRemain)
	{
