...
function routeKoa(linker, ctx, next)
{
	...
	return httpAction(linker, ctx.req)
...
function httpAction(linker, req)
{
	var deprecate_msg = [];
	var client_msg = [];
	...
	return rawBody(req)
		.then(function(buf)
		{
			...
			return runAction(linker, action, body, originalBody)
				.catch(function(err)
				{
...
function runAction(linker, action, body, originalBody)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			if (checkHttpproxyKey(action, originalBody, options) ===  false)
...
function checkHttpproxyKey(action, originalBody, options)
{
	...
	var remain = Date.now() - ckey.time;
	if (!remain || remain > httpproxyKeyRemain || remain < 0)
	{
		debug('[%s] key expired, remain:%sms', action, remain);
		return false;
	}

	var key = signature.signature(action, ckey.time, httpproxyKey);
