...
function runAction(linker, action, startTime, body, headers, originalRaw)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			if (checkHttpproxyTime(linker, action, startTime, contentTime, requestKey, options) === false
...
function checkHttpproxyTime(linker, action, startTime, contentTime, requestKey, options)
{
	if (!contentTime)
	{
