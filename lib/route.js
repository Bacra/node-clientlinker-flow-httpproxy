...
function runAction(linker, action, body, originalBody, headers, startTime)
{
	return linker.parseAction(action)
		.then(function(methodInfo)
		{
			...
			// 如果直接传入httpproxyKey，可以跳过检查
			if (options && options.httpproxyKey && options.httpproxyKey == originalBody.ckey)
			{
				debug('[%s] skip checkHttpproxy', action);
			}
			else if (checkHttpproxyReqRemain(clentRequestTime, startTime, options) === false
